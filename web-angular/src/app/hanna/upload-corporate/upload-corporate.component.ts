...
@Component({
    selector: 'upload-corporate',
    templateUrl: './upload-corporate.component.html',
    styleUrls: ['./upload-corporate.component.css'],
    providers: [UtilService, LOVService, Notification, UploadService, IndividualService, UserSlikService]
})

export class UploadCorporateComponent implements OnInit {
    list: Object[] = [];
    listUser: Object[] = [];
    fileTemp: any;
    ...
    get() {
        this.userSlikService.get().subscribe((data: Object[]) => {
            this.listUser = data;
            this.individualService.getMultiCorporate().subscribe((data: Object[]) => {
                this.evtProses = true;
    ...
    onClose() {
        ...
        this.router.navigate(['/upload-corporate']);
        this.get();
    }
